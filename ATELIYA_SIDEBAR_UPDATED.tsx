...
export default function AteliyaSidebar({ 
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`
        fixed top-0 left-0 h-full bg-gradient-to-b from-teal-50 via-white to-teal-50/30 shadow-2xl shadow-teal-900/10 z-50 transition-all duration-300 ease-in-out border-r border-teal-200
        ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
        ${isCollapsed ? 'w-20' : 'w-64'}
      `}>
        {/* Logo Section */}
        <div className="p-4 border-b border-teal-200 bg-gradient-to-r from-white via-teal-50 to-white backdrop-blur-sm">
          <div className="flex items-center justify-between">
            {!isCollapsed && (
              <div className="flex items-center space-x-3 overflow-hidden">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#53B0B7] via-[#4A9DA4] to-[#3E8A91] flex items-center justify-center shadow-lg shadow-teal-900/30 overflow-hidden">
                  <span className="text-white text-lg font-bold">A</span>
                </div>
                <div>
                  <h1 className="text-lg font-bold bg-gradient-to-r from-teal-900 to-[#53B0B7] bg-clip-text text-transparent">Ateliya</h1>
                  <p className="text-xs text-[#53B0B7]">Gestion de couture</p>
                </div>
              </div>
            )}
            {isCollapsed && (
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#53B0B7] via-[#4A9DA4] to-[#3E8A91] flex items-center justify-center shadow-lg shadow-teal-900/30 overflow-hidden mx-auto">
                <span className="text-white text-lg font-bold">A</span>
              </div>
            )}
            ...
            >
              <svg className="w-5 h-5 text-[#53B0B7]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="hidden lg:block absolute -right-3 top-20 z-10">
          <button
            ...
            className="bg-white hover:bg-teal-50 text-[#53B0B7] hover:text-teal-900 p-1.5 rounded-full shadow-lg shadow-teal-900/20 border-2 border-teal-200 transition-all duration-200 hover:scale-110 hover:border-[#53B0B7]"
              ...
              {item.path ? (
                <Link
                  ...
                  className={`
                    ...
                    ${isActive(item.path)
                      ? 'bg-gradient-to-r from-[#53B0B7] to-[#4A9DA4] text-white shadow-lg shadow-teal-600/30'
                      : 'text-teal-900 hover:bg-teal-100 hover:text-teal-700'
                    }
                    ${isCollapsed ? 'justify-center' : ''}
                  `}
                  title={isCollapsed ? item.title : ''}
                >
                  <div className={`${isActive(item.path) ? 'text-white' : 'text-[#53B0B7] group-hover:text-teal-700'} flex-shrink-0`}>
                    {item.icon}
                  </div>
                  {!isCollapsed && (
                    <>
                  ...
                  {isCollapsed && isActive(item.path) && (
                    <div className="absolute right-1 w-1 h-6 bg-[#53B0B7] rounded-full shadow-lg"></div>
                  )}
                </Link>
              ) : (
                <>
                  <button
                    ...
                    className={`
                      ...
                      ${isParentActive(item)
                        ? 'bg-gradient-to-r from-[#53B0B7] to-[#4A9DA4] text-white shadow-teal-600/30'
                  ...
                  >
                    <div className={`flex items-center space-x-3 ${isCollapsed ? '' : 'flex-1'}`}>
                      <div className={`${isParentActive(item) ? 'text-white' : openMenus.includes(item.title) ? 'text-teal-700' : 'text-[#53B0B7] group-hover:text-teal-700'} flex-shrink-0`}>
                        {item.icon}
                      </div>
                      {!isCollapsed && (
                        <span className="font-medium text-sm">{item.title}</span>
                    ...
                    {!isCollapsed && (
                      <svg
                        className={`w-4 h-4 transition-transform duration-200 flex-shrink-0 ${openMenus.includes(item.title) ? 'rotate-180' : ''} ${isParentActive(item) ? 'text-white' : 'text-teal-400'}`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                  ...
                  {!isCollapsed && openMenus.includes(item.title) && item.subItems && (
                    <div className="mt-1 ml-3 pl-3 border-l-2 border-teal-300 space-y-1">
                        ...
                        <Link
                          ...
                          className={`
                            ...
                            ${isSubItemActive(subItem.path)
                              ? 'bg-gradient-to-r from-teal-300 to-teal-200 text-teal-900 font-semibold shadow-sm'
        ...
        {/* Bottom Section */}
        <div className="absolute bottom-0 left-0 right-0 p-3 border-t border-teal-200 bg-gradient-to-r from-white via-teal-50 to-white backdrop-blur-sm">
          {!isCollapsed ? (
            <div className="flex items-center space-x-3 p-2.5 bg-gradient-to-r from-teal-100 to-teal-50 rounded-xl border border-teal-200 hover:shadow-md hover:border-teal-300 transition-all duration-200">
              <div className="w-9 h-9 bg-gradient-to-br from-[#53B0B7] to-[#3E8A91] rounded-full flex items-center justify-center shadow-lg shadow-teal-900/30 flex-shrink-0">
                <span className="text-white text-sm font-semibold">U</span>
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-teal-900 truncate">Utilisateur</p>
                <p className="text-xs text-[#53B0B7] truncate">{getRoleLabel(userRole)}</p>
              </div>
            </div>
          ) : (
            <div className="flex justify-center">
              <div className="w-9 h-9 bg-gradient-to-br from-[#53B0B7] to-[#3E8A91] rounded-full flex items-center justify-center shadow-lg shadow-teal-900/30">
                <span className="text-white text-sm font-semibold">U</span>
              </div>
            </div>
          )}
